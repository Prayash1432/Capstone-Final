...
for ax, key, title in zip(axes, ['mae', 'rmse'], ['MAE', 'RMSE']):
    ax.plot(history[f'train_{key}'], label=f'Train', linewidth=2)
    ax.plot(history[f'test_{key}'],  label=f'Test',  linewidth=2)
    ax.set(title=title, xlabel='Epoch', ylabel=title)
    ax.legend(); ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'mlp_plot.png'), dpi=300)
plt.close()